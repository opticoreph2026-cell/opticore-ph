      ...
      <div className="bg-background-800 border border-foreground-950/10 rounded-xl overflow-hidden">
        {leads.length === 0 ? (
          <div className="py-12 text-center">
            <Users className="w-8 h-8 text-foreground-50/60 mx-auto mb-3" />
            <p className="text-foreground-50/40 font-medium">No leads assigned yet</p>
            <p className="text-xs text-foreground-50/60 mt-1">Leads assigned by OptiCore will appear here.</p>
          </div>
        ) : (
              ...
              >
                <div className="flex-1 min-w-0 grid grid-cols-1 md:grid-cols-6 gap-3 items-center">
                  <div className="md:col-span-2">
                    <p className="font-medium text-foreground-50">{lead.fullName}</p>
                    <p className="text-xs text-foreground-50/50 capitalize">{lead.customerType.replace(/_/g, ' ')}</p>
                  </div>
                  <div className="text-sm text-foreground-50/40">{lead.email || '—'}</div>
                  <div className="text-sm text-foreground-50/40">{[lead.city, lead.province].filter(Boolean).join(', ') || '—'}</div>
                  <div>
                    <span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium capitalize ${statusColors[lead.status] ?? 'bg-foreground-950/5 text-foreground-50/40'}`}>
                ...
                <ChevronRight className="w-4 h-4 text-foreground-50/60 flex-shrink-0" />
              </Link>
            ))}
          </div>
        )}
