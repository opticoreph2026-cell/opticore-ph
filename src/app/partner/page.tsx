...
export default async function PartnerDashboard() {
  ...
  const activeCount = (projects as any[]).filter((p: any) =>
    ['scheduled', 'in_progress'].includes(p.status)
  ).length;
  const completedCount = (projects as any[]).filter((p: any) =>
    p.status === 'commissioned' || p.status === 'warranty_registered'
  ).length;
  const scheduledCount = (projects as any[]).filter((p: any) =>
    p.status === 'scheduled'
  ).length;
                  ...
                  <tr key={proj.id} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col gap-0.5">
                        {(proj.milestones as any[]).length > 0
                          ? (proj.milestones as any[]).map((m: any, i: number) => (
                              <span key={i} className="text-xs text-gray-500 capitalize">
