    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-white/60 mt-1">Manage all OptiCore platform accounts.</p>
        </div>
            ...
            <tbody className="divide-y divide-border-subtle">
              {clients.map((client: { id: string; email: string; name: string | null; role: string; suspended: boolean; createdAt: Date }) => (
                <tr key={client.id} className="hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="p-4">
                    <span className={`inline-flex items-center justify-center px-2 py-1 rounded-md text-xs font-bold uppercase tracking-wider ${roleColors[client.role] ?? 'bg-white/5 text-white/60 border border-border-subtle'}`}>
