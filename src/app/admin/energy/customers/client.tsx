...
export function CustomerAdminClient({ customers }: { customers: Customer[] }) {
  const columns: Column<Customer>[] = [
    ...
    { key: 'contactEmail', label: 'Email', render: (c) => <span className="text-foreground-50/60">{c.contactEmail || '—'}</span> },
    { key: 'contactPhone', label: 'Phone', render: (c) => <span className="text-foreground-50/60">{c.contactPhone || '—'}</span> },
    { key: 'customerType', label: 'Type', render: (c) => <span className="capitalize">{c.customerType.replace(/_/g, ' ')}</span> },
    { key: 'billingAddress', label: 'Address', render: (c) => <span className="text-foreground-50/60 truncate max-w-[200px] inline-block">{c.billingAddress || '—'}</span> },
