    ...
    { key: 'battery', label: 'Battery', render: (d) => d.battery ? `${d.battery.modelName} (${d.battery.usableKwh}kWh)` : '—' },
    {
      ...
      render: (d) => {
        const colors: Record<string, string> = {
          draft: 'bg-foreground-950/10 text-foreground-50/60',
          finalized: 'bg-accent-emerald/10 text-accent-emerald',
          approved_by_customer: 'bg-accent-cyan/10 text-accent-cyan',
        };
        return (
          <span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium capitalize ${colors[d.status] ?? 'bg-foreground-950/5 text-foreground-50/60'}`}>
