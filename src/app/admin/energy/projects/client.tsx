    ...
    { key: 'org', label: 'Partner', render: (p) => p.organization?.name || '—' },
    {
      ...
      render: (p) => {
        const colors: Record<string, string> = {
          ...
          closed: 'bg-foreground-950/5 text-foreground-50/60',
        };
        return (
          <span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium capitalize ${colors[p.status] ?? 'bg-foreground-950/5 text-foreground-50/60'}`}>
