  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-foreground-950 mb-1">Active Projects</h1>
          <p className="text-sm text-foreground-400">Track solar installation milestones and commissioning status.</p>
        </div>
      </div>
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-foreground-500">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="font-medium text-foreground-400">No active projects</p>
                      <p className="text-xs text-foreground-600">Projects are created when a contract is signed.</p>
                    </div>
                  </td>
                </tr>
              ) : (
                projects.map((project: any) => {
                  ...
                  return (
                    <tr key={project.id} className="hover:bg-foreground-950/5 transition-colors cursor-pointer">
                      ...
                      <td className="px-6 py-4 text-foreground-400">{customer?.siteAddress || 'N/A'}</td>
                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium capitalize ${statusColors[project.status] ?? 'bg-foreground-950/5 text-foreground-400'}`}>
