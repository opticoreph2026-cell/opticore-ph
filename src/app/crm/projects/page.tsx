...
export default async function ProjectsPage() {
  ...
  const statusColors: Record<string, string> = {
    ...
    closed: 'bg-foreground-950/5 text-foreground-50/40',
  };
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-foreground-50/50">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="font-medium text-foreground-50/40">No active projects</p>
                      <p className="text-xs text-foreground-50/60">Projects are created when a contract is signed.</p>
                    </div>
                  </td>
                </tr>
              ) : (
                projects.map((project: any) => {
                  ...
                  return (
                    <tr key={project.id} className="hover:bg-foreground-950/5 transition-colors cursor-pointer">
                      <td className="px-6 py-4 font-medium text-foreground-50">{customer?.fullName || 'Unknown'}</td>
                      <td className="px-6 py-4 text-foreground-50/40">{customer?.siteAddress || 'N/A'}</td>
                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium capitalize ${statusColors[project.status] ?? 'bg-foreground-950/5 text-foreground-50/40'}`}>
