        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-foreground-300">
            ...
            <tbody className="divide-y divide-foreground-950/10">
              {leads.length === 0 ? (
                <tr>
                  <td colSpan={8} className="px-6 py-12 text-center text-foreground-50/50">
                    <div className="flex flex-col items-center gap-3">
                      <svg className="w-10 h-10 text-foreground-50/60" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" />
                      </svg>
                      <p className="font-medium text-foreground-50/40">No leads found</p>
                      <p className="text-xs text-foreground-50/60">Add your first solar prospect to get started.</p>
                    </div>
                  </td>
                </tr>
              ) : (
                      ...
                      <Link href={`/crm/leads/${lead.id}`} className="contents">
                        <div className="flex items-center px-6 py-4">
                          <div className="flex-1 min-w-0 grid grid-cols-7 gap-4 items-center">
                            <div className="font-medium text-foreground-50">
                              <div>{lead.fullName}</div>
                              <div className="text-xs text-foreground-50/50 capitalize">{lead.customerType.replace(/_/g, ' ')}</div>
                            </div>
                            <div>
                              <div>{lead.email || '—'}</div>
                              <div className="text-xs text-foreground-50/50">{lead.phone || '—'}</div>
                            </div>
                            <div className="text-foreground-50/40">{lead.city || '—'}</div>
                            <div>₱{Number(lead.monthlyBill).toLocaleString('en-PH', { minimumFractionDigits: 2 })}</div>
                            <div className="text-foreground-50/40 capitalize">{lead.source.replace(/_/g, ' ')}</div>
                            <div>
                              <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium capitalize ${statusColors[lead.status] ?? 'bg-foreground-950/5 text-foreground-50/40'}`}>
                          ...
                          <ChevronRight className="w-4 h-4 text-foreground-50/60 ml-2 flex-shrink-0" />
                        </div>
                      </Link>
                    </td>
                  </tr>
