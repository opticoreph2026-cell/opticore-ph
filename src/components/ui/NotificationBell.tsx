...
const typeConfig: Record<string, { icon: React.ElementType; color: string; bg: string }> = {
  ...
  system: { icon: Package, color: 'text-foreground-50/60', bg: 'bg-foreground-950/5' },
...
export function NotificationBell() {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <Bell className="w-5 h-5 text-foreground-50/60 group-hover:text-foreground-50 transition-colors" />
        {unread > 0 && (
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-[380px] z-50 bg-background-900 border border-foreground-950/8 rounded-2xl shadow-2xl shadow-black/60 overflow-hidden max-[420px]:fixed max-[420px]:left-2 max-[420px]:right-2 max-[420px]:w-auto">
          <div className="flex items-center justify-between px-5 py-4 border-b border-foreground-950/5">
            <div>
              <h3 className="font-display font-semibold text-foreground-50">Notifications</h3>
              {unread > 0 && (
                <p className="text-xs text-foreground-50/40 mt-0.5">{unread} unread</p>
              )}
            </div>
            <button
              onClick={markAllRead}
          ...
          <div className="max-h-[420px] overflow-y-auto">
            {loading ? (
              <div className="py-12 text-center">
                ...
                <p className="text-xs text-foreground-50/40">Loading...</p>
              </div>
            ) : notifications.length === 0 ? (
              <div className="py-12 text-center">
                <Bell className="w-8 h-8 text-foreground-50/20 mx-auto mb-3" />
                <p className="text-sm text-foreground-50/40">No notifications</p>
              </div>
            ) : fetchError ? (
              <div className="py-12 text-center">
                <AlertCircle className="w-8 h-8 text-accent-rose/50 mx-auto mb-3" />
                <p className="text-sm text-foreground-50/40">Could not load notifications.</p>
                <button
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        <p className={`text-sm font-medium leading-tight ${!notif.isRead ? 'text-foreground-50' : 'text-foreground-50/70'}`}>
                          {notif.title}
                        </p>
                        <button
                          onClick={(e) => {
                          ...
                          className="flex-shrink-0 p-0.5 text-foreground-50/20 hover:text-foreground-50/60 transition-colors"
                        >
                          <X className="w-3 h-3" />
                        </button>
                      </div>
                      <p className="text-xs text-foreground-50/50 mt-1 leading-relaxed line-clamp-2">
                        {notif.message}
                      </p>
                      <p className="text-[10px] text-foreground-50/30 mt-1.5 font-medium">
          ...
          <div className="px-5 py-3 border-t border-foreground-950/5">
            <button
              ...
              className="text-xs text-foreground-50/40 hover:text-foreground-50/70 transition-colors"
            >
