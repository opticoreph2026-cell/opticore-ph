      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={onClose}>
        ...
        >
          <div className="flex items-center justify-between px-6 py-4 border-b border-white/5">
            <h3 className="text-lg font-semibold text-white">{submitLabel}</h3>
          ...
          <div className="flex justify-end gap-3 px-6 py-4 border-t border-white/5">
            ...
            <button
              onClick={onSubmit}
              disabled={saving}
              className="px-5 py-2 text-sm font-medium text-white bg-accent-blue rounded-lg hover:bg-accent-blue/90 disabled:opacity-50 transition-colors"
            >
    ...
    <tr>
      {columns.map((col) => (
        <td key={col.key} className="px-5 py-4">
          <div className="h-4 bg-white/5 rounded animate-pulse" style={{ width: col.width || '80px' }} />
        </td>
      ))}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          ...
          {createFields && onSave && (
            <button
              onClick={openCreate}
              className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-accent-blue rounded-lg hover:bg-accent-blue/90 transition-colors"
            >
            ...
            <tbody className="divide-y divide-white/5">
              {loading ? (
                ...
                paginated.map((item, idx) => (
                  <tr key={item[identifierKey] as string ?? idx} className="hover:bg-white/3 transition-colors">
                    {columns.map((col) => (
                      <td key={col.key} className="px-5 py-4 text-white/80">
                        ...
                        <div className="flex items-center gap-2">
                          {editFields && onSave && (
                            <button
                              ...
                              className="p-1.5 text-white/40 hover:text-accent-cyan transition-colors rounded-lg hover:bg-white/5"
                              title="Edit"
                            >
                          ...
                          {onDelete && (
                            <button
                              ...
                              className="p-1.5 text-white/40 hover:text-accent-rose transition-colors rounded-lg hover:bg-white/5"
                              title="Delete"
                            >
            ...
            <div className="text-center">
              ...
              <div className="flex gap-3 justify-center">
                ...
                <button
                  ...
                  disabled={saving}
                  className="px-5 py-2 text-sm font-medium text-white bg-accent-rose rounded-lg hover:bg-accent-rose/90 disabled:opacity-50 transition-colors"
                >
